...
const validateCase = async (res, body, partial = false) => {
    ...
    if (
        body.firId !== undefined &&
        !(await validateReference(res, FIR, body.firId, "firId"))
    )
        return null;
    if (
        body.assignedOfficerIds !== undefined &&
        !(await validateReferences(
            res,
            Officer,
            body.assignedOfficerIds,
            "assignedOfficerIds",
        ))
    )
        return null;
...
const createCase = async (req, res) => {
    try {
        ...
        if (await Case.exists({ firId: req.body.firId }))
            return conflict(
                res,
        ...
        await record.populate([
            {
                path: "firId",
                select: "firNo date policeStation crimeType status",
            },
            {
                path: "assignedOfficerIds",
...
const getAllCases = async (req, res) => {
    try {
        ...
        const records = await populate(
            Case.find(filter).select("-__v").sort({ createdAt: -1 }),
        );
        return res
            .status(200)
...
const getCaseById = async (req, res) => {
    try {
        ...
        const record = await populate(
            Case.findById(req.params.id).select("-__v"),
        );
        return record
